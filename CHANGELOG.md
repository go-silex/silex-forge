...
- `forge-doctor.sh` exit code is three-way instead of binary: `0` ready (`ok` **and** `deploy_ready`; with `--online`, `online_ok` too), `1` hub/config KO or a local prerequisite failure (missing `lib/`, no `python3`), `2` hub OK but deploy blocked. The human report prints one `→` line per `deploy_blockers` entry naming the command that clears it (token, account id, KV id, `CF_ACCESS_*`, `public_host`, `forge.env` permissions), and `--quiet` now emits exactly one stderr line on any non-zero exit instead of failing silently. The `--json` payload shape is unchanged, and `lib/load_config.py --doctor` keeps its 0/1 library semantics — `forge-doctor.sh` is the product surface. `forge-doctor.sh` output is also fully English (the last French strings are gone).
- `forge-discover.sh --write` persists the confirmed `pages_project` into an **existing** `~/.config/silex/forge.config.json`, so `--project NAME` is needed once rather than on every call. It never creates that file: creation belongs to the `forge-setup` skill, and doctor's `no local config` issue has to stay meaningful. `forge.env` remains credentials-only. Exit `0` with a non-empty `missing[]` now prints a concrete follow-up command per absent key, and `unparsed` no longer suggests `--project` (which cannot fix a parse failure) nor provisioning. Exit codes stay `0` / `1` / `2`.
...
- `.env.example` ships no `PUBLIC_HOST` value (the key is documented, commented out) and keeps `FORGE_PAGES_PROJECT` commented: `forge.config.json` is the single source of truth for the host and the project name. Copying the example was previously taught as *the* machine-config path, which meant a client's first configured host was `forge.gosilex.com`.
...
- `forge-provision.sh` could create the Access **Bypass** policies without a verified fail-closed deploy: a missing `x-forge-acl` header at stage 10 only triggered a `Continue anyway?` prompt, and answering yes reached stage 11 — publishing every artifact on the account. The header must now be present and equal `vis-v4`, which makes stage 11 unreachable otherwise; the escape prompt is deleted. An unreadable `wrangler pages project list` aborts instead of offering to create a project (matching `forge-discover.sh`, which never provisions on an unparsed list), the wizard refuses a non-TTY run so `yes | forge-provision.sh` cannot auto-confirm twelve stages, stage 9 forces only `hub_root` / `public_host` / `pages_project` into `forge.config.json` so a re-run no longer clobbers a healthy config or empties `vault_markers`, and a failed `chmod 600` on `forge.env` is fatal rather than best-effort.
