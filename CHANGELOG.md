...
- Dropped the undocumented `?cacheTTL=0` query parameter from the Browser Run request. Measured against the live API on 2026-09-08: the endpoint answers `HTTP 400 Unrecognized key: "cacheTTL"` when it is passed in the body and accepts it in the query string, where it is documented nowhere (absent from the screenshot endpoint page, the Quick Actions index and `llms.txt`). Nothing needs it. The ~5 s response cache is keyed on the request body — a repeat of the identical probe body replayed the same bytes and the same `X-Browser-Ms-Used` at 0.15 s wall against 3.8 s for the render, so the second `--online` of a `/forge-setup` run is free — and a cache hit is still authenticated: the same cached body with an invalid token answers `HTTP 401`, not a cached `200`. Scope of that measurement: authenti*cation*, not authori*zation* — a token that stays valid but loses `Browser Rendering · Edit` answers `403`, and whether that also precedes the cache was not tested, so a probe repeated within ~5 s of a permission change could in principle read the previous green. That measurement also corrects the probe's cost: one render is 0.13–2.5 s of browser time depending on how warm the instance is, not the 133 ms first observed, which was itself a cache hit.
